...
const Divider = () => <div className="h-4 w-px bg-background-dark flex-shrink-0" />;
...
const EditorToolbar = ({
  ...
  const toolButtonClass = (tool: string) =>
    `w-7 h-7 flex items-center justify-center rounded-[8px] text-sm transition-colors active:scale-90 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500/60 ${
      activeTool === tool
  ...
  return (
    <header className="flex-shrink-0 border-b border-surface-border bg-surface-dark/60">
      {/* Ярус 1 — навигация, идентификация, действия над схемой */}
      <div className="flex items-center gap-3 px-4 h-12">
        <button
          type="button"
          onClick={onBack}
          className="flex items-center gap-1.5 text-sm text-text-muted hover:text-text-primary transition-colors rounded-[8px] active:scale-95 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500/60"
        >
        ...
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-sm font-semibold text-text-primary truncate max-w-[18rem]">
            {title}
          </span>
          {isDirty ? (
            <span className="flex-shrink-0 flex items-center gap-1 h-5 px-1.5 rounded-[8px] border border-amber-500/40 bg-amber-500/10 text-[12px] font-ibmPlexMono tracking-wide uppercase text-amber-300">
              <span className="w-1 h-1 rounded-full bg-amber-400" />
              не сохранено
            </span>
          ) : (
            <span className="flex-shrink-0 h-5 px-1.5 rounded-[8px] border border-surface-border text-[12px] font-ibmPlexMono tracking-wide uppercase text-text-faint leading-5">
              сохранено
            </span>
          )}
        </div>
      ...
      <div className="flex items-center gap-2.5 px-4 h-9 border-t border-surface-border/70 bg-background-dark/30">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-[8px] text-text-secondary transition-colors hover:bg-background-dark enabled:active:scale-90 disabled:opacity-30 disabled:hover:bg-transparent focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500/60"
          >
            ↶
          </button>
          <button
        ...
        <div className="flex items-center gap-0.5">
          <button
            ...
            onClick={() => stepZoom(-1)}
            title="Уменьшить"
            className="w-6 h-6 flex items-center justify-center rounded-[8px] text-text-muted hover:bg-background-dark hover:text-text-primary transition-colors active:scale-90 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500/60"
          >
            −
          </button>
          <button
            ...
            className="w-12 h-6 rounded-[8px] text-[13px] text-text-muted font-ibmPlexMono tabular-nums hover:bg-background-dark hover:text-text-primary transition-colors active:scale-95 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500/60"
          >
            {Math.round(zoom * 100)}%
          </button>
          <button
        ...
        <div className="flex items-center gap-1">
          <div className="flex items-center rounded-[8px] border border-surface-border overflow-hidden">
          ...
          <button
            ...
            className={`h-6 px-2 rounded-[8px] border text-[13px] font-ibmPlexMono uppercase tracking-wide transition-colors active:scale-95 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500/60 ${
              snapToGrid
