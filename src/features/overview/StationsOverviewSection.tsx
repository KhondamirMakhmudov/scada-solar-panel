...
const HeroTile = ({
  ...
  >
    <div className="flex items-center gap-2.5 mb-3">
      ...
      <p className="text-[14px] font-ibmPlexSans font-semibold uppercase tracking-wider text-text-muted">
        {label}
      </p>
    </div>
    <p className="text-text-primary text-[42px] font-ibmPlexSans font-semibold leading-none">
      {value}
    </p>
  </motion.div>
);
    ...
    <section className="space-y-3">
      <div className="flex items-baseline justify-between">
        ...
        <span className="flex items-center gap-1.5 text-[13px] font-ibmPlexMono text-text-faint">
          ...
          Обновлено{" "}
          {formatRelativeToNow(response.generatedAt, new Date().toISOString())}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <HeroTile
          ...
          label="Мощность сейчас"
          value={formatPower(response.totals.energy.currentPowerW)}
          delay={0}
        />
        <HeroTile
          icon={TodayRounded}
          label="Выработка сегодня"
          value={formatEnergy(response.totals.energy.todayKwh)}
          delay={0.05}
        />
        <HeroTile
          icon={BatteryChargingFullRounded}
          label="Выработка всего"
          value={formatEnergy(response.totals.energy.totalKwh)}
          delay={0.1}
        />
      </div>
      ...
      >
        <p className="text-[13px] font-ibmPlexSans uppercase tracking-wide text-text-faint mb-3">
          ...
          {response.totals.stations === 1 ? "станции" : "станциях"}
        </p>
