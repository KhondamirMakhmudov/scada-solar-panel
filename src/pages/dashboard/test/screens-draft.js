...
import { TableRows, ViewModule, KeyboardArrowDown, Sell, Cable, Memory } from "@mui/icons-material";
...
import { requestPython, requestScreensDraft } from "@/services/api";

// Черновая версия страницы «Экраны»: те же CRUD-операции (создать,
// изменить, клонировать, удалить), но записи создаются в дубликате сервиса
// экранов (8103), а не в проде (8102) — можно тестировать создание экранов,
// не рискуя рабочими данными. Без редактора мнемосхемы и живого просмотра:
// это отдельная, гораздо более объёмная задача (редактор жёстко завязан на
// прод-клиент requestScreens в нескольких местах фичи mnemonic-editor).
...
const TagTreeSelect = ({ label, tree = [], value = [], onChange }) => {
  ...
  const labelById = useMemo(() => {
    const map = new Map();
    tree.forEach((conn) =>
      conn.devices.forEach((dev) => dev.tags.forEach((tag) => map.set(tag.id, tag.label))),
    );
    return map;
  }, [tree]);
        ...
        const devices = conn.devices
          .map((dev) => {
            ...
            const tags =
              connMatch || devMatch
                ? dev.tags
                : dev.tags.filter((tag) => tag.label.toLowerCase().includes(normalizedQuery));
            return { ...dev, tags };
          })
  ...
  const countSelected = (tags) => tags.reduce((acc, tag) => acc + (value.includes(tag.id) ? 1 : 0), 0);
          ...
          {visibleTree.map((conn) => (
            <div key={conn.id}>
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-background-dark active:bg-surface-border/40 transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500/60 focus-visible:ring-inset"
              >
                <KeyboardArrowDown
                  sx={{ fontSize: 18 }}
                  className={`text-text-dim transition-transform ${isExpanded(conn.id) ? "" : "-rotate-90"}`}
                />
                <Cable sx={{ fontSize: 15 }} className="text-blue-300" />
                <span className="flex-1 text-left font-medium text-text-primary truncate">{conn.label}</span>
                <span className="text-xs text-text-dim">
                  {conn.devices.reduce((acc, dev) => acc + countSelected(dev.tags), 0)}/
              ...
              {isExpanded(conn.id) &&
                conn.devices.map((dev) => {
                  ...
                  const allSelected = dev.tags.length > 0 && selectedCount === dev.tags.length;
                  ...
                  return (
                    <div key={dev.id}>
                      ...
                      >
                        <KeyboardArrowDown
                          sx={{ fontSize: 18 }}
                          className={`text-text-dim transition-transform ${isExpanded(dev.id) ? "" : "-rotate-90"}`}
                        />
                        <input
                          type="checkbox"
                          checked={allSelected}
                        ...
                        />
                        <Memory sx={{ fontSize: 15 }} className="text-emerald-300" />
                        <span className="flex-1 text-left text-text-secondary truncate">{dev.label}</span>
                        <span className="text-xs text-text-dim">
                        ...
                        dev.tags.map((tag) => (
                          ...
                          >
                            <input
                              ...
                              className="cursor-pointer"
                            />
                            <Sell sx={{ fontSize: 13 }} className="text-cyan-300" />
...
const ScreenCard = ({ screen, isSelected, onSelect, onEditDetails, onOpenDetails, onClone, onDelete, canUpdate, canDelete }) => {
...
const ScreensDraftPage = () => {
  ...
  // Черновой список экранов — отдельный ключ кэша, не пересекается с
  // прод-списком на /dashboard/screens.
  const {
    data: screensResp,
    isLoading: isLoadingScreens,
    isFetching: isFetchingScreens,
  } = useGetQuery({
    key: [KEYS.screens, "draft"],
    url: URLS.screens,
  ...
  // Теги/устройства/подключения не дублированы на черновом бэкенде —
  // берём как есть с прода, для дерева выбора тегов.
  const { data: tagsResp } = useGetQuery({
    key: [KEYS.tags, "screens-draft"],
    url: URLS.tags,
  ...
  const { data: devicesResp } = useGetQuery({
    key: [KEYS.devices, "screens-draft"],
    url: URLS.devices,
    apiClient: requestPython,
    headers: { Authorization: `Bearer ${session?.accessToken}` },
    enabled: !!session?.accessToken,
  ...
  const { data: connectsResp } = useGetQuery({
    key: [KEYS.connects, "screens-draft"],
    url: URLS.connects,
    apiClient: requestPython,
    headers: { Authorization: `Bearer ${session?.accessToken}` },
    enabled: !!session?.accessToken,
  ...
  const connectsRaw = get(connectsResp, "data.data", get(connectsResp, "data", []));
  const connectsList = Array.isArray(connectsRaw) ? connectsRaw : [];

  const tagTree = useMemo(() => {
    const deviceById = new Map(devicesList.map((dev) => [dev.id, dev]));
    const connNameById = new Map(connectsList.map((conn) => [conn.id, conn.name || conn.id]));

    const groups = new Map();

    for (const tag of tagsList) {
      const tagNode = { id: tag.id, label: tag.name ? formatTagLabel(tag.name) : tag.id };
      ...
      if (!groups.has(connKey)) {
        groups.set(connKey, {
          id: connKey,
          label: connId ? connNameById.get(connId) || connId : "Без подключения",
          devices: new Map(),
        });
      }
    ...
    return [...groups.values()].map((group) => ({ ...group, devices: [...group.devices.values()] }));
  ...
  const handleCreateScreen = () => {
    ...
    createScreen(
      ...
      {
        onSuccess: () => {
          toast.success("Черновой экран создан");
          setShowCreateModal(false);
          resetCreateForm();
        },
        onError: (error) => {
          toast.error(translateApiError(get(error, "response.data.message")) || "Ошибка создания экрана");
        },
      },
    );
  };
  ...
  const handleUpdateScreen = async () => {
    ...
    try {
      ...
      await requestScreensDraft.patch(
        `${URLS.screens}/${editingScreen.id}`,
        buildPayload(editForm, editingScreen.params || {}),
        { headers: authHeaders },
      );

      toast.success("Черновой экран обновлён");
      queryClient.invalidateQueries({ queryKey: [KEYS.screens, "draft"] });
      setShowEditModal(false);
      setEditingScreen(null);
      setEditErrors({});
    } catch (error) {
      toast.error(translateApiError(get(error, "response.data.message")) || "Ошибка обновления экрана");
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  const handleDeleteScreen = () => {
    ...
    deleteScreen(
      ...
      },
      {
        ...
        onError: (error) => {
          toast.error(translateApiError(get(error, "response.data.message")) || "Ошибка удаления экрана");
        },
      },
    );
  };
  ...
  const handleCloneScreen = (screen) => {
    createScreen(
      ...
      },
      {
        ...
        onError: (error) =>
          toast.error(translateApiError(get(error, "response.data.message")) || "Не удалось клонировать экран"),
      },
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Обновлено",
      cell: ({ row }) => <span className="text-text-secondary">{formatDate(row.original.updatedAt)}</span>,
    },
    {
      id: "actions",
      header: "Действия",
  ...
  return (
    <DashboardLayout headerTitle="Экраны (черновик)">
      <div className="font-ibmPlexSans space-y-2.5">
        ...
        <div style={{ background: "#1c1b1b", border: "1px solid #2a2a2a" }}>
              ...
              {VIEW_MODE_OPTIONS.map((item, idx) => {
                ...
                return (
                  <button
                    ...
                    className="font-ibmPlexMono transition-colors active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70 focus-visible:ring-inset"
                    style={{
                      padding: "5px 11px",
                      cursor: "pointer",
                      fontSize: 10,
          ...
          {filteredList.length === 0 ? (
            <NoData title="Экраны не найдены" description="Измените фильтры или создайте новый экран." />
          ) : viewMode === "table" ? (
          ...
          <div className="mt-2.5 flex flex-col items-center justify-between gap-3 border-t border-surface-border pt-3 sm:flex-row">
            ...
            <div className="flex items-center gap-1">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                ...
                .map((item, idx) =>
                  item === "..." ? (
                    <span key={`ellipsis-${idx}`} className="flex h-7 w-7 items-center justify-center text-text-dim">
                      …
                    </span>
                  ) : (
                    <button
            ...
            <span className="text-[11px] text-text-muted">
              ...
              {" · "}
              <span className="font-semibold text-text-primary">{filteredList.length}</span> записей
            </span>
          </div>
        )}
      </div>
        ...
        <div className="space-y-3 font-mono">
          ...
          <div className="pt-2 flex items-center justify-end gap-2">
            <Button
              ...
              sx={{ textTransform: "none", color: "#cbd5e1", borderColor: "#475569" }}
              variant="outlined"
            >
              Отмена
            </Button>
            <Button
              ...
              sx={{ textTransform: "none", background: "#2563eb", color: "#eff6ff", "&:hover": { background: "#1d4ed8" } }}
              variant="contained"
            >
      ...
      <MethodModal
        ...
        showCloseIcon={true}
        title="Редактировать черновой экран"
        width={700}
      >
        <div className="space-y-3 font-mono">
          <Input
          ...
          <Input
            ...
            onChange={(event) => handleChangeEditField("description", event.target.value)}
            placeholder="Описание экрана"
          />
          ...
          <div className="pt-2 flex items-center justify-end gap-2">
            ...
            <Button
              ...
              disabled={isUpdating}
              sx={{ textTransform: "none", background: "#ea580c", color: "#fff7ed", "&:hover": { background: "#c2410c" } }}
              variant="contained"
            >
...
export default ScreensDraftPage;
